...
![Replicator Image](img/replicator_view.png)  
*Image showcasing the general layout used for domain randomized data generation with Replicator.*   
...
**yolo_test.py** - Loads a specified trained detection model and a list of image paths, runs each of the images through the detection model, draws the result, and saves the result image to a specified output directory. This is mainly for qualitative testing and doesn't assume to have any ground truth for accuracy metrics.  

**benchmark_yolo_model.py** - This assumes you have a benchmarking dataset and generates accuracy metrics using a trained YOLO model.    
